...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	DPrintf("[ ----Server[%v]----] : send a Get,op is :%+v \n", kv.me, op)
	...
	case replyOp := <-ch:
		...
		if op.ClientId != replyOp.ClientId || op.SeqId != replyOp.SeqId {
			...
			DPrintf("[ ----Server[%v]----] : set reply.Val = %v \n", kv.me, kv.kvPersist[args.Key])
			kv.mu.Unlock()
			return
		}
	case <-timer.C: // 超过100ms就会触发该条件
		reply.Err = ErrWrongLeader
	}
}
...
func (kv *KVServer) PutAppend(args *PutAppendArgs, reply *PutAppendReply) {
	...
	DPrintf("[ ----Server[%v]----] : is sending a %v,op is :%+v \n", kv.me, args.Op, op)
	lastIndex, _, _ := kv.rf.Start(op)
	...
	select {
	case replyOp := <-ch:
		DPrintf("[ ----Server[%v]----] : receive a %vAsk :%+v,Op:%+v\n", kv.me, args.Op, args, replyOp)
...
// form the fault-tol
//
//	// 设置超时tickererant key/value service.
//
...
// 判断是否是重复操作的也比较简单,因为是对seq进行递增，所以直接比大小即可

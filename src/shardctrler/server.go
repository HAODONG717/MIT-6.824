...
func (sc *ShardCtrler) Join(args *JoinArgs, reply *JoinReply) {
	...
	op.Index = lastIndex
	DPrintf("server接收到join请求:%v", sc.printOp(op))
...
func (sc *ShardCtrler) Query(args *QueryArgs, reply *QueryReply) {
	...
	case replyOp := <-ch:
		...
		if op.ClientId != replyOp.ClientId || op.SeqId != replyOp.SeqId {
			...
			DPrintf("我是节点%d, 现在打印返回配置: %v", sc.me, sc.printOp(replyOp))
			reply.WrongLeader = false
			return
		}
	case <-timer.C:
		...
		case msg := <-sc.applyCh:
			...
			if msg.CommandValid {
				...
				if !sc.ifDuplicate(op.ClientId, op.SeqId) {
					...
					switch op.OpType {
					case JoinOp:
						//DPrintf("节点%d执行join操作", sc.me)
						sc.execJoinCmd(&op) // 注意一定要传地址，而不是值
						//DPrintf("put后，结果为%v", sc.kvPersist[op.Key])
					case MoveOp:
						sc.execMoveCmd(&op)
						//DPrintf( "Append后，结果为%v", sc.kvPersist[op.Key])
					case QueryOp:
						sc.execQueryCmd(&op)
					case LeaveOp:
						sc.execLeaveCmd(&op)
				...
				if _, isLead := sc.rf.GetState(); isLead {
					DPrintf("我是节点%d, 通知leader接收信息: %v", sc.me, sc.printOp(op))
